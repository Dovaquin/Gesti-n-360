...
const Dashboard: React.FC = () => {
  const { user, transactions, loading } = useStore();
  const navigate = useNavigate();

  // Si los datos aún están cargando de la nube
  if (loading) {
    return (
      <div className="flex flex-col h-screen items-center justify-center bg-background-dark text-white">
        <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin mb-4"></div>
        <p className="text-white/50 animate-pulse font-bold tracking-widest uppercase text-xs">Sincronizando Cloud...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background-dark text-white pb-6">
      {/* Top Bar con indicador de sincronización */}
      <div className="flex items-center p-4 justify-between sticky top-0 z-20 bg-background-dark/95 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <Link 
            to="/settings"
            className="w-10 h-10 rounded-full bg-cover bg-center border-2 border-primary/20 hover:border-primary transition-all"
        ...
        <div className="w-10 flex justify-end">
          <Link to="/settings" className="text-white/70 hover:text-primary transition-colors">
      ...
      <div className="flex flex-col gap-3 p-4 pt-2">
        <div className="grid grid-cols-2 gap-3">
            <Link to="/transactions?type=SALE" className="flex flex-col justify-between gap-1 rounded-2xl p-4 bg-surface-dark border border-white/5 shadow-lg hover:bg-[#23482f] transition-colors active:scale-[0.98]">
                ...
                <div>
                    ...
                    <p className="text-primary text-[10px] font-bold bg-primary/10 w-fit px-2 py-0.5 rounded-full mt-1">Este Mes</p>
                </div>
            </Link>
            ...
            <Link to="/transactions?type=EXPENSE" className="flex flex-col justify-between gap-1 rounded-2xl p-4 bg-surface-dark border border-white/5 shadow-lg hover:bg-[#23482f] transition-colors active:scale-[0.98]">
                ...
                <div>
                    ...
                    <p className="text-orange-400 text-[10px] font-bold bg-orange-400/10 w-fit px-2 py-0.5 rounded-full mt-1">Este Mes</p>
                </div>
            </Link>
        </div>
        ...
        <div className="flex flex-col gap-2 rounded-2xl p-5 bg-surface-dark border border-white/5 shadow-lg relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 text-white/70 relative z-10">
            ...
            <span className="text-sm font-medium">Balance Cloud</span>
          </div>
          ...
          <div className="flex items-center gap-2 relative z-10">
             ...
             <p className="text-white/50 text-xs font-medium">Total Sincronizado</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        <h2 className="text-xl font-bold">Actividad en Nube</h2>
        <Link to="/transactions" className="text-primary text-sm font-semibold hover:underline">Ver Historial</Link>
      </div>
      
      <div className="px-4">
        <div className="flex flex-col rounded-2xl bg-surface-dark p-5 border border-white/5">
      ...
      {/* Accesos Rápidos */}
      <h2 className="text-xl font-bold px-4 pt-6 pb-3">Menús Principales</h2>
      <div className="grid grid-cols-2 gap-4 px-4 pb-12">
        <QuickAction to="/transaction/new" icon="add_shopping_cart" label={<>Nueva<br/>Transacción</>} hasPermission={!!user?.permissions.sales} />
        <QuickAction to="/inventory" icon="inventory_2" label="Inventario" hasPermission={!!user?.permissions.inventory} />
        <QuickAction to="/customers" icon="groups" label="Clientes" hasPermission={!!user?.permissions.customers} />
        <QuickAction to="/reports" icon="analytics" label="Reportes" hasPermission={!!user?.permissions.reports} />
        <QuickAction to="/users" icon="manage_accounts" label="Gestión Equipo" hasPermission={user?.role === 'admin'} fullWidth={true} />
      </div>
    </div>
  );
};
